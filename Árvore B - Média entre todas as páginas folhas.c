...
int media_folha (NodeB* tree, int *soma, int *cont){
    int elem=0;
    ...
    if(tree != NULL){ 
        if(tree->eh_no_folha){
            ...
            *cont += tree->nro_chaves;
        }
        
        while(elem < N){
            NodeB *filho = tree->filhos[elem];
            
            if(filho != NULL){
                media_folha(filho, soma, cont);
            }
...
int main() {
    ...
    for(i=0; i<num; i++){
        scanf("%d", &val);
        tree = inserir(tree, val);
    }
    
    int soma=0, cont=0;
    media_folha(tree, &soma, &cont);
    printf("%d", soma/cont);
